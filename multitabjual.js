...
window.addEventListener('DOMContentLoaded', () => {
    const savedTabs = JSON.parse(localStorage.getItem('formPenjualanTabs') || '[]');
    const savedIndex = parseInt(localStorage.getItem('formPenjualanActiveTab') || '0', 10);
...
function createNewTab() {
    ...
    localStorage.setItem('formPenjualanActiveTab', currentTabIndex);
    saveTabs();
    renderTabs();
    clearForm();
...
function switchTab(index) {
    ...
    localStorage.setItem('formPenjualanActiveTab', index);
    loadFormFromTab();
    restoreStatusUI();
    renderTabs();
}
...
function loadFormFromTab() {
    ...
    dataPenjualan = [...(tabData.dataPenjualan || [])];
...
function saveCurrentForm() {
    ...
    tabs[currentTabIndex].dataPenjualan = [...dataPenjualan];
...
function saveTabs() {
    localStorage.setItem('formPenjualanTabs', JSON.stringify(tabs));
}
...
function removeTab(index) {
    ...
    if (tabs.length === 0) {
        ...
        localStorage.removeItem('formPenjualanActiveTab');
        clearForm();
    } else {
...
// ======== BERSIHKAN FORM ========
function clearForm() {
    const formElements = document.querySelectorAll('#formPenjualan input, #formPenjualan select, #formPenjualan textarea');
