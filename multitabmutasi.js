...
window.addEventListener('DOMContentLoaded', () => {
    const savedTabs = JSON.parse(localStorage.getItem('formMutasiTabs') || '[]');
    const savedIndex = parseInt(localStorage.getItem('formMutasiActiveTab') || '0', 10);
...
function createNewTab() {
    ...
    localStorage.setItem('formMutasiActiveTab', currentTabIndex);
    saveTabs();
    renderTabs();
    clearForm();
...
function switchTab(index) {
    ...
    localStorage.setItem('formMutasiActiveTab', index);
    loadFormFromTab();
    restoreStatusUI();
    renderTabs();
}
...
function saveCurrentForm() {
    ...
    tabs[currentTabIndex].dataMutasi = [...dataMutasi];
...
function saveTabs() {
    localStorage.setItem('formMutasiTabs', JSON.stringify(tabs));
}
...
function removeTab(index) {
    ...
    if (tabs.length === 0) {
        ...
        localStorage.removeItem('formMutasiActiveTab');
        clearForm();
    } else {
...
// ======== BERSIHKAN FORM ========
function clearForm() {
    const formElements = document.querySelectorAll('#formMutasi input, #formMutasi select, #formMutasi textarea');
